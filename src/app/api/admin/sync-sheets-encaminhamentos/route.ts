...
function buildRow(e: Encaminhamento, fup: Followup | undefined): (string | number)[] {
  ...
  const profIndicado = prof ? prof.nome : (e.profissional_solicitado ?? "");
...
export async function POST(request: Request) {
  ...
  const sheetId = process.env.GOOGLE_SHEETS_ENCAMINHAMENTOS_ID;
  if (!saJson || !sheetId) {
    return NextResponse.json({ error: "Variáveis GOOGLE_SERVICE_ACCOUNT_JSON ou GOOGLE_SHEETS_ENCAMINHAMENTOS_ID não configuradas na Vercel" }, { status: 500 });
  }
  ...
  const [encRes, fupRes] = await Promise.all([
    fetch(`${supabaseUrl}/rest/v1/encaminhamentos?profissional_solicitado=not.is.null&order=criado_em.desc&limit=1000&select=id,criado_em,nome_responsavel,contato,cidade,modalidade,profissional_solicitado,observacoes,status`, { headers }),
